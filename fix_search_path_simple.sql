-- Simple script to fix search_path for existing functions only
-- Ye script sirf un functions ko fix karega jo actually exist karte hain

-- Check karne ke liye query
SELECT 'ALTER FUNCTION public.' || proname || '() SET search_path = '''';' as fix_command
FROM pg_proc 
...
-- Actual fix commands (run these one by one)
-- Pehle upar wali query run karo, jo functions exist karte hain sirf unke liye commands copy karo

-- Extension fix
CREATE SCHEMA IF NOT EXISTS extensions;
-- Note: pg_trgm extension ko move karne ke liye superuser privileges chahiye
-- Agar error aaye to Supabase support se contact karo