...
const PurchaseReturnList = ({ returns, onEdit, onDelete, loading, totalPages, currentPage, onPageChange, searchTerm, setSearchTerm, dateRange, setDateRange, onExport }) => {
            ...
            <CardContent>
                <div className="flex flex-col md:flex-row gap-4 mb-4">
                    <div className="relative flex-grow">
                        <Input
                            placeholder="Search by Chassis No, Engine No, Party Name..."
                            value={searchTerm}
                             ...
                             returns.length > 0 ? returns.map(r => (
                                <motion.tr key={r.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                        ...
                                        <ul className="list-disc list-inside">
                                            {(Array.isArray(r.items) ? r.items : []).map((item, index) => (
                                                <li key={index} className="text-xs">
                                                    {item.modelName} ({item.chassisNo} / {item.engineNo})
                                                </li>
                                            ))}
                                        </ul>
                                    </TableCell>
                                    ...
                                    <TableCell>
                                        ...
                                        {canAccess('purchase_returns', 'delete') && (
                                            <Button variant="ghost" size="icon" className="text-red-500" onClick={() => onDelete(r)}><Trash2 className="w-4 h-4" /></Button>
                                        )}
                                    </TableCell>
                                </motion.tr>
                            )) : <TableRow><TableCell colSpan="6" className="text-center">No purchase returns found for the selected criteria.</TableCell></TableRow>}
                            </AnimatePresence>
                        </TableBody>
                    </Table>
                </div>
